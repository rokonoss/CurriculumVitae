...
var omnitureEnv = function () {
    var dev = "seekemploymentdeve,seekglobaldeve",
        prod = "seekemployment,seekglobalprod",
        host = location.host;

    var suffix = /(\.dev)|(\.staging)|(\.stress\d+)|(\.web-.+)|(\.vm-.+)|(\..+-dev)|(\..+-qa)$/;

    return suffix.test(host) ? dev : prod;
};

var s = s_gi(omnitureEnv());
/************************** CONFIG SECTION **************************/
...
s.linkInternalFilters = "javascript:,.seek.com,//seek.com,http://seek.com.au,.seek.com.au,seek.co.nz,seeklearning.com.au,seeklearning.com,seeklearning.co.nz,//jobsearch.gov.au,.jobsearch.gov.au"
...
/** IE8: Fixes < IE8  not supporting indexOf in arrays */
...
function getQueryParamCheckOriginalURL(s, paramName) {
    var param = this.s.getQueryParam(paramName);
    if (!param && s['originalURL']) {
        var regPattern = new RegExp('[?&]' + paramName + '=([^&]*)', 'i');
        var matchParam = s['originalURL'].match(regPattern);
        if (matchParam) param = matchParam[0].substr(paramName.length + 2);
    }
    return param;
}
...
s.setBusinessValues = function (obj) {
    /* Mapping Logic */
    obj.hier1 = s.businessUnit + '|' + s.country + '|' + s.platform + '|' + s.section;
    obj.channel = s.platform;
    obj.prop1 = s.businessUnit + ':' + s.country;
    obj.prop2 = s.prop1 + ':' + s.platform;
    obj.prop3 = s.prop2 + ':' + s.section;

    if (obj.pageName) obj.pageName = obj.prop2 + ':' + obj.pageName;    //TODO: check with JED if this is correct

    return obj;
};

function s_doPlugins(s) {
    s = s.setBusinessValues(s);
    /* START - Customized by Analytics Team ==================================================================== */
    s.server = window.location.host;

    /* Internal Campaign Tracking */
    if (!s.eVar7) {
        s.eVar7 = getQueryParamCheckOriginalURL(s, 'cid');
        if (s.eVar7) {
            s.eVar8 = s.crossVisitParticipation(s.eVar7, 's_ev8', '0', '5', '>', '');
            if (s.eVar7) { s.events = s.apl(s.events, "event2", ",", 1); }
        }
    }

    /*always set the internal campaign pathing*/
    s.prop8 = s.eVar7 + ":" + s.pageName; //internal campaign pathing

    /* External Campaign Tracking */
    if (!s.campaign) {
        s.campaign = getQueryParamCheckOriginalURL(s, 'tracking') //capture the cmp url string parameter into the s.campaign variable

        if (s.campaign) {
            // s.campaign=s.getValOnce(s.campaign,'s_ev0',0); //capture external promotion codes once
            s.clickPast(s.campaign,'event56','event57'); //click through quality
            s.eVar59=s.crossVisitParticipation(s.campaign,'s_ev59','30','5','>',''); //cross visit participation of external promotions
        }else if (document.referrer){
            var seoBrand;
            var ref_domain = document.referrer;
            if(ref_domain.indexOf('google') > -1 || ref_domain.indexOf('yahoo') > -1 || ref_domain.indexOf('bing') > -1){
                        if (ref_domain.indexOf('seek') > -1) {
                            seoBrand = "seo:ske"; 
                        } else {
                            seoBrand = "seo:non";
                        }
            s.eVar59=s.crossVisitParticipation(seoBrand,'s_ev59','30','5','>',''); //cross visit participation of external promotions
            }
            }else{
            s.eVar59=s.crossVisitParticipation('direct','s_ev59','30','5','>',''); //cross visit participation of external promotions
            }			
    }
    
    /*always set the external campaign pathing*/
    s.prop59=s.campaign+":"+s.pageName; //external campaign pathing
    
    

    /* Time to complete registration */
    if(s.events && s.events.indexOf('event12')>-1) s.ttc='start';
    if(s.events && s.events.indexOf('event16')>-1) s.ttc='stop';
    s.prop46=s.getTimeToComplete(s.ttc,'ttc',0);
    ...
    s.eVar61=s.getTimeParting('h','+10'); // Set hour 
    s.eVar62=s.getTimeParting('d','+10'); // Set day 

    /* Set the new and repeat visitor value */
    s.eVar64=s.getNewRepeat(); //new and repeat visitors

        
    /* Days since last visit never*/
    s.eVar65=s.getDaysSinceLastVisit();
    
    /* Visit number */
    s.eVar66=s.getVisitNum(365);


    /* Set Custom page view event*/
    if(!s.events){
    s.events="event74"; //custom page view event
    }else{
    s.events=s.apl(s.events,"event74",",",2);
    }	

    /* JobMail ProfileID */
    if (!s.eVar56) s.eVar56 = getQueryParamCheckOriginalURL(s, 'savedSearchID');
                
    /* ExactTarget Vis ID */
    if (!s.eVar57) s.eVar57 = getQueryParamCheckOriginalURL(s, 'etrid');

    /* ExactTarget CampaignID */
    if (!s.eVar58) {
        s.eVar58 = getQueryParamCheckOriginalURL(s, 'etcid');
        s.eVar58 = s.getValOnce(s.eVar58, 'ev58', 0);
    }
    
    /* Search Type Stacking */
     if (!s.eVar33 && s.prop32) { s.eVar33 = s.crossVisitParticipation(s.prop32, 's_nvtypcvp', '1', '5', '>','', 0); } 

    /* Dynamic Variable for Page URL */
        s.prop5=s.eVar5="D=g"; 


    /* Omniture visitor cookie s_vi*/
                s.prop10=s.eVar10="D=s_vi";  

    /* SearchSurvey Page CalledFrom - fired on prop69 */
    if(s.prop69){
        //s.prop68=getPrev;
    }

    /* Copy Props to eVars */
    if(s.prop1 &&! s.eVar1) s.eVar1="D=c1";//BusinessUnit Country
    if(s.prop2 &&! s.eVar2) s.eVar2="D=c2";//BusinessUnit Country Platform
    if(s.prop3 &&! s.eVar3) s.eVar3="D=c3";//BusinessUnit Country Platform Label
    if(s.prop4 &&! s.eVar4) s.eVar4="D=c4";//Page Title
    if(s.prop6 &&! s.eVar6) s.eVar6="D=c6";//Page Type
    if(s.prop9 &&! s.eVar9) s.eVar9="D=c9";// Internal Menu Interaction
    if(s.prop11 &&! s.eVar11) s.eVar11="D=c11";//Login User name
    if(s.prop12 &&! s.eVar12) s.eVar12="D=c12";//Login state
    if(s.prop13 &&! s.eVar13) s.eVar13="D=c13";//Candidate Dashboard
    if(s.prop14 &&! s.eVar14) s.eVar14="D=c14";//Candidate JSP Status  |JSP Privacy
    if(s.prop16 &&! s.eVar16) s.eVar16="D=c16";//Profile Guid
    if(s.prop17 &&! s.eVar17) s.eVar17="D=c17";//Job Position in Search Results
    if(s.prop21 &&! s.eVar21) s.eVar21="D=c21";//Search  Terms
    if(s.prop22 &&! s.eVar22) s.eVar22="D=c22";//Search Classification
    if(s.prop23 &&! s.eVar23) s.eVar23="D=c23";//Search Sub Classification
    if(s.prop24 &&! s.eVar24) s.eVar24="D=c24";//Search Work Type
    if(s.prop25 &&! s.eVar25) s.eVar25="D=c25";//Search Salary Range
    if(s.prop26 &&! s.eVar26) s.eVar26="D=c26";//Search Company
    if(s.prop27 &&! s.eVar27) s.eVar27="D=c27";//Search Country State
    if(s.prop28 &&! s.eVar28) s.eVar28="D=c28";//Search Location
    if(s.prop29 &&! s.eVar29) s.eVar29="D=c29";//Search Area
    if(s.prop30 &&! s.eVar30) s.eVar30="D=c30";//Search From When
    if(s.prop31 &&! s.eVar31) s.eVar31="D=c31";//Sort by Refine
    if(s.prop32 &&! s.eVar32) s.eVar32="D=c32";//Search Type
    if(s.prop35 &&! s.eVar35) s.eVar35="D=c35";//Search Suburb
    if(s.prop36 &&! s.eVar36) s.eVar36="D=c36";//Search results
    if(s.prop37 &&! s.eVar37) s.eVar37="D=c37";//Search job auto inclusion
    if(s.prop38 &&! s.eVar38) s.eVar38="D=c38";//Testing Recipe ID V2
    if(s.prop40 &&! s.eVar40) s.eVar40="D=c40";//searchid
    if(s.prop41 &&! s.eVar41) s.eVar41="D=c41";//JobId	
    if(s.prop42 &&! s.eVar42) s.eVar42="D=c42";//JobType
    if(s.prop47 &&! s.eVar47) s.eVar47="D=c47";//SupportType
    if(s.prop49 &&! s.eVar49) s.eVar49="D=c49";//JobClass
    if(s.prop53 &&! s.eVar53) s.eVar53="D=c53";//Tiered location SERP info
    if(s.prop63 &&! s.eVar63) s.eVar63="D=c63";//SERP Location Tier
    if(s.prop69 &&! s.eVar69) s.eVar69="D=c69";//SurveyName
    if(s.prop70 &&! s.eVar70) s.eVar70="D=c70";//Surveyresp
    if(s.prop71 &&! s.eVar71) s.eVar71="D=c71";//Trial recipe
    if(s.campaign &&! s.eVar60) s.eVar60="D=v0";//30Day Campaign
...
/* Plugin: TNT Integration v1.0 */ s.trackTNT=new Function("v","p","b","" +"var s=this,n='s_tnt',p=p?p:n,v=v?v:n,r='',pm=false,b=b?b:true;if(s." +"getQueryParam){pm=s.getQueryParam(p);}if(pm){r+=(pm+',');}if(s.wd[v" +"]!=undefined){r+=s.wd[v];}if(b){s.wd[v]='';}return r;");


/* WARNING: Changing any of the below variables will cause drastic
changes to how your visitor data is collected.  Changes should only be
made when instructed to do so by your account manager.*/
s.visitorNamespace="seek"
...
s.clickEvent = function (options) {
    s.clearValues();
    
    var clickEvent = s_gi(omnitureEnv());
    clickEvent.linkTrackVars = "prop1,eVar1,prop2,eVar2," + options.linkTrackVars;
    clickEvent.linkTrackEvents = options.linkTrackEvents;
    
    for (var propertyName in options.properties) {
        clickEvent[propertyName] = options.properties[propertyName];
    }
    
    clickEvent = s.setBusinessValues(clickEvent);
    clickEvent.tl(this, 'o', options.name);
    
    s.clearValues();
};

/* Example of using s.clickEvent
var clickEventData = {
    name: 'SocialShareLink',
    linkTrackVars: 'eVar50,events',
...
s.sendAjaxTracking = function(trackingArray) {
    ///<summary>
    /// Loops through <paramref name="trackingArray"/> and sets all the 's' properties to the findings.
    /// Then creates an omniture tracking object which is executed and sent to omniture tracking servers.
    ///</summary>
    if (!trackingArray)
        return false;
    ...
    for (var propertyName in trackingArray) {
        s[propertyName] = trackingArray[propertyName];
    }

    var trackingBeacon = s.t();
    if (trackingBeacon) {
        document.write(trackingBeacon);
    }

    return true;
};

/* Example of using s.sendAjaxTracking
var ajaxTrackingArray = {
...
s.sendAjaxTracking(ajaxTrackingArray);
*/
