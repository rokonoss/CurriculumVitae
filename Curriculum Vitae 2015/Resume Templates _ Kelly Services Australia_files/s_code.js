...
/*var s_account="kellyservicescom"
Remove from this file and set the value for each site separately.

made the change to file pwe Dan on 1/7/2013
*/
/*Updates by Adobe Consultant - Sagar Mandal on 11/13/2014 :
Code updated to : 27.4
eVar50 allocated to contain social campaign tracking code
prop52 contains the business vertical
prop53 contains the region of the website
*/
...
s.linkInternalFilters = "javascript:,kellyservices,kellyglobal.com,kellytemps.com,kellygirl.com,kellyglobal.net"
...
function s_doPlugins(s) {
    ...
    //    s.eVar30 = dbs.audience 			||defaultVal;
    //    s.eVar31 = dbs.audience_segment	||defaultVal;
    //    s.eVar35 = dbs.industry 			||defaultVal;
    //    s.eVar38 = dbs.employee_range 	||defaultVal;
    //    s.eVar37 = dbs.revenue_range 	||defaultVal;
    //    s.eVar34 = dbs.company_name 		||defaultVal;
    /********* Demandbase Plugin ******** */
    ...
    for (var i = 0; i < temp.length; i++) {
        if (temp[i] != "" && pgName != "empty") pgName = pgName + " : " + temp[i]
        else
            if (pgName == "empty") pgName = temp[i]
    }
    s.pageName = pgName
    s.pageName = s.pageName.toLowerCase();
    s.pageName = s.pageName.split("#")[0];
    s.pageName = s.pageName.split("?hid")[0];

    /* Nested Conditional Loop to populate Social campaign */

    if (s.getQueryParam('sid'))
        s.eVar50 = s.getQueryParam('sid');
    ...
    var link1 = s.linkHandler('.exe|.zip|.wav|.mp3|.mov|.mpg|.avi|.wmv|.doc|.docx|.pdf|.xls|.xlsx|.ppt|.pptx|.gz|.swf', '/getFile', 'd');
    if (link1) {
        s.eVar6 = s.prop12 = link1
        s.prop13 = s.eVar7 = s.pageName;
        s.events = "event6";
        s.linkTrackVars = "prop12,eVar6,prop13,eVar7,events";
        s.linkTrackEvents = "event6";
    }

    // search code
    if (s.getQueryParam('searchtext')) {
        s.prop25 = s.getQueryParam('searchtext')
        s.eVar8 = s.prop25.toLowerCase(); //internal search keywords
        var t_search = s.getValOnce(s.eVar8, 'ev8', 0);

        if (document.getElementById('ctl00_ctl00_cphSubScripts_cphAnalytics_Omniture_hfResultCount').value != 0) {
            if (t_search) s.events = s.apl(s.events, 'event18', ',', 1); // searches
        }
        else {
            if (t_search) s.events = s.apl(s.events, 'event19', ',', 1); // null search
        }
    }
...
/*Demandbase integration code start */
...
s.Integrate.onLoad = function (s, m) {
    ...
    */
    ...
    _db._key = "46f79c3e274a0be774721ecb1830e54f";
...
/*demandbase integration code end */
...
/*
* Utility: split v1.5 - split a string (JS 1.0 compatible)
*/
...
s.trackingServer = "kellyservices.112.2o7.net"
/* demandbase integrate module start */
...
s.m_i("Integrate");
/* demandbase integrate module end */
